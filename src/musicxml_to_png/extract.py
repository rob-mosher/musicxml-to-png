...
def extract_notes(
    ...
    for part_index, part in enumerate(score.parts, start=1):
        ...
        for element in part.recurse().notes:
            ...
            voice_id = str(voice_ctx.id) if voice_ctx is not None and voice_ctx.id is not None else None
        ...
        for i, (pitch_midi, offset, duration, original_duration, tie_type, element, voice_id) in enumerate(note_data):
            ...
            for idx, ev in instrument_notes:
                ...
                for lane_idx, (end_time, last_pitch, last_orig_end) in enumerate(lane_meta):
                    ...
                    if end_time <= ev.start_time + CONNECTION_TIME_EPS or same_underlying:
                        lane_meta[lane_idx] = (ev.start_time + ev.original_duration, ev.pitch_midi, ev.start_time + ev.original_duration)
                        lane_notes[lane_idx].append((idx, ev))
                        assigned = True
                        break
                if not assigned:
                    lane_meta.append((ev.start_time + ev.original_duration, ev.pitch_midi, ev.start_time + ev.original_duration))
